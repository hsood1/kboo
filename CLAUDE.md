...
**`nyc-food` is JavaScript-free by design** — its interactivity (His/Hers and bagel toggles) is built with the **CSS checkbox/radio hack**: hidden `<input>`s near the top of `<body>` drive visibility through `~` sibling selectors. The hidden inputs must stay ahead of the content they control, or the selectors silently stop working. Don't reintroduce JS rendering on nyc-food (it's shared as a raw file / printed). See `nyc-food/NYC_FOOD.md` for the rationale.

**`car-search` is JavaScript-driven** — it was rewritten from the old CSS-only version into a small JS app (JS is fine here: it's served over HTTPS on GitHub Pages, not `file://`). All car data lives in one `CARS` array in the `<script>` at the bottom of `<body>`; the Cards page, the top-3 picks on the Start page, and the sortable Compare table all render from it, so **edit a car once and every view updates**. It's tuned for a non-technical phone user: the default tab is a friendly **🏁 Start here** page (top picks + plain-English guidance), the four filters (Mode/Style/Power/Budget → `getFiltered()`) are **collapsed behind a `🔍 Filter cars` toggle** so browsing is the default, and there's a single merged **Compare all** table (Score is its first column). `showPage()`/`showPageById()` switch tabs; `toggleCard()` expands a card; `resetFilters()` backs every empty-state "Show all cars" button. Keep pinch-zoom enabled (no `maximum-scale` in the viewport). See `car-search/CAR_SEARCH.md` for the data model and buyer context.
