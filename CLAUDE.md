...
**`car-search` is JavaScript-driven** — it was rewritten from the old CSS-only version into a small JS app (JS is fine here: it's served over HTTPS on GitHub Pages, not `file://`). All car data lives in one `CARS` array in the `<script>` at the bottom of `<body>`; both the Cards page and the sortable Master table render from it, so **edit a car once and both views update**. Four AND-combined filter rows (Mode / Style / Power / Budget) drive `getFiltered()`; `showPage()` switches the six tabs; `toggleCard()` handles card expand. See `car-search/CAR_SEARCH.md` for the data model and buyer context.
